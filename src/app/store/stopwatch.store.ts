    ...
    withMethods((store, stopwatchService = inject(StopwatchService)) => ({
        ...
        loadStopwatch: rxMethod<{ id: string; ignoreTsArch: boolean }>(
            pipe(
                switchMap(({ id, ignoreTsArch }) =>
                    from(stopwatchService.fetchStopwatch(id, ignoreTsArch)).pipe(
                        tap((stopwatch) => {
        ...
        addStopwatch: rxMethod<void>(
            pipe(
                ...
                switchMap(() =>
                    from(stopwatchService.addStopwatch()).pipe(
                        tap((stopwatch) =>
            ...
            pipe(
                ...
                switchMap(({ stopwatchId, newRound, isStart }) =>
                    from(stopwatchService.addEvent(stopwatchId, newRound, isStart)).pipe(
                        tap((events) =>
                            updateState(store, Actions.addStopwatchEvent, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) => {
                        ...
                        catchError((error) => {
                            updateState(store, Actions.addStopwatchEvent, {
                                loading: false,
                                error,
                            });
                            return [];
        ...
        updateStopwatchTitle: rxMethod<{ stopwatch: Stopwatch; title: string }>(
            pipe(
                ...
                switchMap(({ stopwatch, title }) =>
                    from(stopwatchService.updateStopwatch(stopwatch, { name: title })).pipe(
                        tap((updatedStopwatch) =>
                            updateState(store, Actions.updateStopwatchTitle, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) =>
        ...
        toggleArchiveStopwatch: rxMethod<{ stopwatch: Stopwatch; tsArch: number }>(
            pipe(
                ...
                switchMap(({ stopwatch, tsArch }) =>
                    from(stopwatchService.toggleArchiveItem(stopwatch, tsArch)).pipe(
                        tap((updatedStopwatch) =>
                            updateState(store, Actions.toggleArchiveStopwatch, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) =>
                                    sw._id === updatedStopwatch._id ? updatedStopwatch : sw,
                                ),
        ...
        deleteStopwatch: rxMethod<Stopwatch>(
            pipe(
                tap(() =>
                    updateState(store, Actions.deleteStopwatch, { loading: true, error: null }),
                ),
                switchMap((stopwatch) =>
                    from(stopwatchService.deleteStopwatch(stopwatch)).pipe(
                        tap((resp) =>
                            updateState(store, Actions.deleteStopwatch, (state) => ({
                                loading: false,
        ...
        updateStopwatchEvent: rxMethod<{ event: StopwatchEvent; ts: number }>(
            pipe(
                ...
                switchMap(({ event, ts }) =>
                    from(stopwatchService.updateEvent(event, { ts })).pipe(
                        tap((updatedEvent) =>
                            updateState(store, Actions.updateStopwatchEvent, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) => {
                                    if (sw._id === updatedEvent.ref) {
                                        return {
                                            ...sw,
                                            events: sw.events.map((ev) =>
                        ...
                        catchError((error) => {
                            updateState(store, Actions.updateStopwatchEvent, {
                                loading: false,
                                error,
                            });
                            return [];
        ...
        updateStopwatchEventLabel: rxMethod<{ event: StopwatchEvent; label: string }>(
            pipe(
                ...
                switchMap(({ event, label }) =>
                    from(stopwatchService.updateEvent(event, { name: label })).pipe(
                        tap((updatedEvent) =>
                            updateState(store, Actions.updateStopwatchEventLabel, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) => {
                                    if (sw._id === updatedEvent.ref) {
                                        return {
                        ...
                        catchError((error) => {
                            updateState(store, Actions.updateStopwatchEventLabel, {
                                loading: false,
                                error,
                            });
                            return [];
        ...
        deleteStopwatchEvent: rxMethod<StopwatchEvent>(
            pipe(
                tap(() =>
                    updateState(store, Actions.deleteStopwatchEvent, {
                        loading: true,
                        error: null,
                    }),
                ),
                switchMap((event) =>
                    from(stopwatchService.removeEvent(event)).pipe(
                        tap((resp) =>
                            updateState(store, Actions.deleteStopwatchEvent, (state) => ({
                                loading: false,
                                stopwatches: state.stopwatches.map((sw) => {
                                    if (sw._id === event.ref) {
                                        return {
